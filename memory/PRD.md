...
- Line stroke animation (user-mandated spec): 4 SVG paths fill="none" with stroke-dasharray from getTotalLength(); stroke-dashoffset animated full-length→0 in continuous CSS keyframe loop (`lineflow` on @property --dashloop, staggered 14/17.5/21/24.5s + negative delays) AND tied to scroll via GSAP ScrollTrigger scrub:true animating --scrolloff, combined via calc(). gsap dependency added.
