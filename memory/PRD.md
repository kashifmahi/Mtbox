...
- Multi-language (June 2026): EN/FR/DE/ES/PT/AR via custom i18n — /app/frontend/src/i18n/LanguageContext.jsx (React context, localStorage 'mbtex-lang', sets document lang + dir=rtl for Arabic) + translations/{en,fr,de,es,ar}.js (full site copy). All components consume useLang(); switcher pills in navbar + mobile menu (data-testid lang-switch-*). RTL CSS adjustments + Tajawal/Amiri fonts for Arabic in App.css. Verified: all languages render, AR is RTL, persistence on reload.
...
## Backlog / Next
- Branding: official MBtex horizontal VECTOR logo (user-provided SVG, /app/frontend/public/mbtex-group-horizontal-vector.svg) in navbar + footer; its globe section replaced with the previous globe (tab-icon art, base64-embedded raster) per user request; older PNG kept as fallback asset. Navbar logo enlarged to h-[88px] desktop / h-[64px] mobile (header h-24) per user request (Jun 2026)
